...
import io.github.architectplatform.engine.core.project.interfaces.dto.CommandDTO
import io.github.architectplatform.engine.core.project.interfaces.dto.CommandResponseDTO
...
class ProjectsApiController(
	...
	@Error(global = true)
	fun handleError(exception: Exception): CommandResponseDTO {
	...
	fun getAll(@PathVariable projectName: String): List<CommandDTO> {
		...
		return commands.map { it.toDTO() }.also { println("Commands found: $it") }
	}

	@Get("/{projectName}/commands/{commandName}")
	fun getCommand(@PathVariable projectName: String, @PathVariable commandName: String): CommandDTO {
		...
		return command.toDTO().also { println("Command found: $it") }
	}
	...
	fun executeCommand(
		...
		return commandResult.toDTO().also { println("Command result: $it") }
	}
	...
	fun getContext(@PathVariable projectName: String): ContextDTO {
		val project = projectService.getProject(projectName)
			?: throw IllegalArgumentException("Project not found: $projectName")
		return project.context.toDTO().also { println("Context found: $it") }
	}
